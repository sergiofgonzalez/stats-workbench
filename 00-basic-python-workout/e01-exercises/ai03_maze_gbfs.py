...
if __name__ == "__main__":
    if (len(sys.argv) < 2 or len(sys.argv) > 3 or
        (len(sys.argv) == 3 and
            ...
            sys.argv[2].upper() != Maze.GREEDY_BREADTH_FIRST_SEARCH)):
        sys.exit("Usage: python ai02_maze_harvard.py path/to/maze.txt [dfs|bfs|gbfs]\nNote: gbfs is the default search mode")

    m = Maze(sys.argv[1], search_mode=Maze.GREEDY_BREADTH_FIRST_SEARCH if len(sys.argv) == 2 else sys.argv[2].upper())
    print("Maze:")
    m.print()
