...
class GbfsFrontier(StackFrontier):
    ...
    def manhattan_distance_to_goal(self, node):
        cur_row, cur_col = node.state
        goal_row, goal_col = self.goal_state
        md = abs(goal_row - cur_row) + abs(goal_col - cur_col)
        return md

    def remove(self):
        if self.empty():
            raise Exception("Cannot remove from empty BgfsFrontier")
        else:
            node = min(self.frontier, key=lambda node: self.manhattan_distance_to_goal(node))
            node_pos = self.frontier.index(node)
            self.frontier = self.frontier[:node_pos] + self.frontier[node_pos + 1:]
            return node
...
class AstarFrontier(StackFrontier):
    ...
    def cost_to_reach_node(self, node: Node) -> int:
        return node.cost

    def remove(self):
        if self.empty():
            raise Exception("Cannot remove from empty AstarFrontier")
        else:
            node = min(self.frontier, key=lambda node: self.cost_to_reach_node(node) + self.manhattan_distance_to_goal(node))
            node_pos = self.frontier.index(node)
            self.frontier = self.frontier[:node_pos] + self.frontier[node_pos + 1:]
            return node
...
class Maze:
    ...
    GREEDY_BREADTH_FIRST_SEARCH = "GBFS"
    ...
    def solve(self):
        ...
        elif self.search_mode == Maze.BREADTH_FIRST_SEARCH:
            frontier = QueueFrontier()
        elif self.search_mode == Maze.GREEDY_BREADTH_FIRST_SEARCH:
            frontier = GbfsFrontier(self.goal)
        elif self.search_mode == Maze.A_STAR_SEARCH:
            frontier = AstarFrontier(self.goal)
        else:
            raise Exception(f"Unknown search mode {self.search_mode}")
        frontier.add(start)
...
if __name__ == "__main__":
    if (len(sys.argv) < 2 or len(sys.argv) > 3 or
        (len(sys.argv) == 3 and
            sys.argv[2].upper() != Maze.DEPTH_FIRST_SEARCH and
            sys.argv[2].upper() != Maze.BREADTH_FIRST_SEARCH and
            sys.argv[2].upper() != Maze.GREEDY_BREADTH_FIRST_SEARCH and
            sys.argv[2].upper() != Maze.A_STAR_SEARCH)):
        sys.exit("Usage: python ai02_maze_harvard.py path/to/maze.txt [dfs|bfs|gbfs|a_star]\nNote: a_star is the default search mode")

    m = Maze(sys.argv[1], search_mode=Maze.A_STAR_SEARCH if len(sys.argv) == 2 else sys.argv[2].upper())
    print("Maze:")
    m.print()
